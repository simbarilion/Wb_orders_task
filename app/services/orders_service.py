...
class OrdersService:
    ...
    def build_top_articles_message(top_df: pd.DataFrame) -> str:
        """
        Формирует текстовое сообщение с топ‑3 артикулами по количеству заказов.
        Создаёт структурированное сообщение, где каждый артикул сопровождается количеством заказов.
        Args:
            top_df: pd.DataFrame - DataFrame с колонками 'article' (артикул) и 'count' (количество заказов).
        Returns:
            str: Отформатированное текстовое сообщение для отправки в Telegram.
            Пример:
